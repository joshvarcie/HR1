...
# Collapse to totals for compliance group
ohp_elig_total <- ohp_elig |>
  mutate(num_american_indian = american_indian*PERWT, 
         num_w2 = w2*PERWT,
         num_self_employed = self_employed*PERWT,
         num_likely_gig = likely_gig*PERWT,
         num_ui_receipt = ui_receipt*PERWT,
         num_education_programs = education_programs*PERWT,
         num_income_compliant = income_compliant*PERWT) |> 
  summarize(across(starts_with("num"), ~sum(.x, na.rm = TRUE))) |> 
  t() |> 
  as.data.frame()

ohp_elig_total$group <- rownames(ohp_elig_total)
rownames(ohp_elig_total) <- NULL

ohp_elig_pct <- ohp_elig_total |> 
  filter(group != "num_disabled_in_hh_minus_self") |> 
  rename(group_total = V1) |> 
  mutate(pct = group_total/total_eligible)
